...
# Function to load features from a signal
def get_feature(
                        signal,
                        sample_rate,
                        add_channel_dim,
    ...
    if (add_channel_dim):
        if (feature_type.lower() not in ['rawsignal']):
            ...
            if (row_nb > 0):
                ...
                if ('V' not in row[sample_name_column].split('.')[-1]):
                    ...
                    if (patient_ID not in patient_ids_old_new):
                        patient_ids_old_new[patient_ID] = new_patient_ID
                        new_patient_ID += 1
                #patient_ID = int(row[sample_name_column].split('.')[0][1:])
                #print("Original Patient ID: ", row[sample_name_column].split('.')[0])
                #print("\tInt Patient ID: ", patient_ID)
                #break
                ...
                # WARNING: NORMALIZATION IS DONE HERE !!!!
